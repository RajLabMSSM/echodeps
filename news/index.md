...
- Standardize `rworkflows.yml` with canonical template: replace
  `run_crancheck` with `run_rcmdcheck` + `as_cran`, enable Docker on
  `ghcr.io`, set `write-all` permissions, use `GITHUB_TOKEN`, add
  `devel`/`RELEASE_**` branch triggers.
...
- New function `construct_colors`: helps construct colors for
  `dep_graph_plot`.
- New function `dep_graph_create` to create just the graph without the
  plot.
- New function `subset_graph` to easily subset graphs by node names.

### Bug fixes

- Fixed `package_metadata` example by using
  `echogithub::description_find` to more robustly get *DESCRIPTION*
  file.
